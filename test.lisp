...
(defmacro passert (i p &optional (r 'result) (s 'matchp) (e 'endp))
  `(multiple-value-bind (result matchp endp)
       (parse ,i ,p)
     (assert ,r () "Failed assertion: ~a for RESULT = ~a" ',r result)
     (assert ,s () "Failed assertion: ~a for MATCHP = ~a" ',s matchp)
     (assert ,e () "Failed assertion: ~a for ENDP = ~a" ',e endp)))
...
(defun test-fail ()
  ...
  (passert-n "a" (%maybe (?fail)) (null result) matchp)
  (passert "" (%maybe (=element)) (null result)))

(defun test-atom ()
  (passert-n "foo" (=element) (eq #\f result) matchp)
  (passert "" (=element) (null result) (null matchp)))
...
(defun test-any ()
  (passert "" (%any (=element)) (null result) matchp)
  (passert-n "foo123" (=subseq (%any (?satisfies 'alpha-char-p)))
             (equal result "foo") matchp)
  (passert "" (%some (=element)) (null result) (null matchp))
...
(defun test-or ()
  (let ((fo (%or (?eq #\f) (?eq #\o))))
    ...
    (passert-n "x" fo)
    (passert "" fo (null result) (null matchp))))
...
(defun test-transform ()
  (passert '(16) (=list (=transform (=element) 'sqrt) (?end))
           (equal result '(4 nil)))
  (passert '(:a :b :c :d :e :f)
...
(defun test-char ()
  ...
  (passert (coerce (append *whitespace* '(#\a)) 'string)
...
(defun test-digit ()
  ...
  (passert "1234a" (=integer-number) (= result 1234) matchp (null endp)))
