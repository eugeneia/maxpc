...
(defun test-backtracking ()
  (passert "a" (?either (?eq #\a) (?eq #\b)) (null result))
  (passert "b" (?either (?eq #\a) (?eq #\b)) (null result))
  (passert "." (?optional (?eq #\.)) (null result))
  (passert "" (?optional (?eq ".")) (null result))
  (passert "0aaaaaaaa1"
           (?path (?eq #\0) (?all (?satisfies 'alphanumericp)) (?eq #\1))
           (null result))
  (passert "aaac"
           (?path (?all (?either (?string "aa") (?string "aaa") (?string "c")))
                  (?end))
           (null result))
  (let ((domain-like
         (?either
          (?path (?all (?path (?all (?satisfies 'alphanumericp))
                 ...
                 (?end))
          (?seq (?eq #\.) (?end)))))
    (passert "." domain-like (null result))
    (passert "foo." domain-like (null result))
    (passert "1foo.bar" domain-like (null result))
    (passert "foo.b2ar.baz" domain-like (null result))
    (passert "foo.bar.2baz." domain-like (null result))
    (passert-n "foo2" domain-like)
    (passert-n ".." domain-like)
    (passert-n "123.456" domain-like)))
